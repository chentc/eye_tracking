...
void jnilog(char *str) { 
        int fd = open("/sdcard/eye_tracking/log/logs.txt", O_RDWR | O_APPEND | 
O_CREAT, 0666); 
        if(fd > -1) { 
...
int opendevice(int i)
{
	...
	if (ret !=0) {
		LOGE("Could not aaa %s : %d, %s",system_call_buffer, errno, strerror (errno));
		return ERROR_LOCAL;
	}
...
JNIEXPORT void JNICALL Java_com_camera_simplewebcam_CameraPreview_processCamera( JNIEnv* env,
										jobject thiz){
...
JNIEXPORT void JNICALL Java_com_camera_simplewebcam_CameraPreview_pupilDetection(JNIEnv* env, jobject thiz){
	...
	int ratio = 2;
	int kernel_size = 3;
	Mat mrgb(height, width,CV_8UC4, (unsigned char *)rgb);
	Mat mgray(height, width,CV_8UC4);
	cvtColor(mrgb, mgray, CV_RGB2GRAY);
	Mat medges;
	
	/*Mat mintegral;
	integral(mgray,mintegral);
	jnilog("cccc\n");
	// note
	//float* aintegral = (float*)mintegral.data;
	Mat reshaped = mintegral.reshape(1,1);
	Mat reshapedInFloat;
	reshaped.convertTo(reshapedInFloat, CV_32F);

	float* aintegral = (float*)(reshapedInFloat.data);
	
	int rows = mintegral.rows;
	int cols = mintegral.cols;
    int x, y, w;
    
    float response;
    filter(aintegral,rows,cols,&x,&y,&w,100,400,&response);
    char a[20],b[20],ax[20],ay[20],aw[20];
    sprintf(a,"rows:%d\n",rows);
	sprintf(b,"cols:%d\n",cols);
	sprintf(ax,"x:%d\n",x);
	sprintf(ay,"y:%d\n",y);
    ...
    jnilog(ay);
    jnilog(aw);
    Point point1 = Point(y,x);
    Point point2 = Point(y+w,x+w);
    jnilog("b\n");
    int coarse_pupil_width = w/2;
	int padding = coarse_pupil_width/4;
	Rect roi = Rect(point1.x,point1.y,point2.x-point1.x,point2.y-point1.y);
	
	Mat pupil_img = mgray(roi);
	jnilog("d\n");
	int offset = 11;
	int spectral_offset = 5;
	jnilog("aaaaa11\n");*/
    Canny(mgray, medges, 50, 150, 3,false);
  	/*jnilog("bbbbb\n");
  	//findNonZero(medges,medges);
  	jnilog("e\n");
  	mrgb = mrgb(roi);
  	jnilog("f\n");
  	char roi_width [10];
	char roi_height [10];
	sprintf(roi_width,"mrgb.rows:%d\n",mrgb.rows);
	jnilog(roi_width);
	sprintf(roi_height,"mrgb.cols:%d\n",mrgb.cols);
	jnilog(roi_height);
	char edge_width [10];
	char edge_height [10];
	sprintf(edge_width,"medges.rows:%d\n",medges.rows);
	jnilog(edge_width);
	sprintf(edge_height,"medges.cols:%d\n",medges.cols);
	jnilog(edge_height);*/
	mrgb.setTo(Scalar(0, 128, 255, 255), medges);
	jnilog("f\n");
	rgb = (int*)mrgb.data;
 }
